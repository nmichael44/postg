...
import app.MemCaches.MemCache2.{hasExpired, CacheElem, CacheState}
import org.typelevel.log4cats.Logger

final class MemCache2[F[_]: { Temporal as temporal, Logger as logger }, K: Ordering, V] private (
    memCacheName: String,
    capacity: Int,
  ...
  private def checkDuration(durationOpt: Option[java.time.Duration]): F[Unit] =
    durationOpt match {
      case Some(d) if d.compareTo(MemCache2.ItemMinimumAllowedDuration) < 0 =>
        temporal.raiseError(
          new IllegalArgumentException(
            s"MemCache2.put(): Duration cannot be less than ${TimeUtils.durationToString(MemCache2.ItemMinimumAllowedDuration)}.",
  ...
  private def stopCleanupFiber(): F[Unit] =
    logger.info(s"Stopping MemCache2 '$memCacheName' cleanup worker...") *>
      cleanupFiber.cancel *>
      logger.info(s"Stopping MemCache2 '$memCacheName' timeTick worker...") *>
      timeTickFiber.cancel *>
      logger.info(s"MemCache2 '$memCacheName' workers stopped.")
  end stopCleanupFiber
...
object MemCache2:
  ...
  private def ensureMinCleanupDuration[F[_]: Temporal as temporal](cleanupDuration: FiniteDuration): F[Unit] =
    (cleanupDuration < MinimumCleanupDuration).whenA(
      temporal.raiseError(
        new IllegalArgumentException(
          s"MemCache2: Cleanup duration cannot be less than ${TimeUtils.durationToString(MinimumCleanupDuration)}.",
        ) with NoStackTrace,
      ),
    )
  ...
  private def ensureMinTimeTickDuration[F[_]: Temporal as temporal](timeTickDuration: FiniteDuration): F[Unit] =
    (timeTickDuration < MinimumTimeTickDuration).whenA(
      temporal.raiseError(
        new IllegalArgumentException(
          s"MemCache2: TimeTick duration cannot be less than ${TimeUtils.durationToString(MinimumTimeTickDuration)}.",
        ) with NoStackTrace,
      ),
    )
  ...
  private def create[F[_]: { Temporal as temporal, Logger as logger }, K: Ordering, V](
    ...
    } yield MemCache2(memCacheName, capacity, r, cleanupFiber, timeTickingFiber)
  end create
  ...
  private def timeTickWorker[F[_]: { Temporal as temporal, Logger as logger }, K: Ordering, V](
    ...
    val logResettingClock = logi("Resetting internal memCache2 clock!")

...
object ImplicitConversions:
  extension [A](nev: NonEmptyVector[A]) inline def view: View[A] = nev.toVector.view
  ...
  extension [F[_]: Functor, A](fa: F[A]) inline def lifte[B]: EitherT[F, B, A] = EitherT.liftF[F, B, A](fa)

  extension [F[_], A, B](fe: F[Either[A, B]]) inline def toEitherT: EitherT[F, A, B] = EitherT(fe)

  extension [F[_]: Applicative, A, B](e: Either[A, B]) inline def toEitherT: EitherT[F, A, B] = EitherT.fromEither(e)
